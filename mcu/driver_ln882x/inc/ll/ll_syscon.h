 ...
 */
uint32_t LL_SYSCON_IdleRegGet(void);

void LL_SYSCON_IdleRegSet(uint32_t val);
 ...
 *
 */
void LL_SYSCON_GPIO_NoPull(GPIO_Num gpio_num);
...
void LL_SYSCON_DigSel_Set(uint16_t bitmap);

uint16_t LL_SYSCON_DigSel_Get(void);
...
void LL_SYSCON_XtalVol_Set(uint8_t Vol);
