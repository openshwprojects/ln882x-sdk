...
/**
 * @brief Read data from RX FIFO and store in the buffer.
 *
 * @param i2cX
 * @param buf
 * @param length
...
/**
 * @brief Controls whether to issue START/STOP signal, whether to read or send the data.
 *
 * @param i2cX
 * @param restart Issue a START signal before send data.
 * @param stop Issue a STOP signal before send data.
 * @param read_write_mode 1 for READ mode, 0 for WRITE mode.
 * @param data Ignore this data and try to read data to RX FIFO if in READ mode, otherwise send this data.
 */
void HAL_I2C_Data_Cmd_Pack(I2CInstance *i2cX, uint8_t restart, uint8_t stop, uint8_t read_write_mode, uint8_t data);

/**
 * @brief Send command to read multiple data and store in the buffer, and then issue STOP signal
 * to the slave device automatically.
 *
 * @param i2cX
...
/**
 * @brief Send command to read multiple data and then issue STOP signal to slave device automatically.
 * The data is stored in RX FIFO, try to read the actual data from RX FIFO.
 *
 * @param i2cX
 * @param length
 */
...
/**
 * @brief Send command to read one byte and then issue STOP signal to slave device automatically.
 * The data is stored in RX fifo, try to read the actual data from RX FIFO.
 * @param i2cX
 */
...
/**
 * @brief Send an array of data to slave device, then send STOP signal automatically.
 *
 * @param i2cX
 * @param buf
 * @param length
 * @return uint32_t return 0 on success, others on failure.
 */
uint32_t HAL_I2C_WriteData(I2CInstance *i2cX, uint8_t *buf, uint32_t length);

/**
 * @brief Send last byte to slave device, then send STOP signal automatically.
 *
 * @param i2cX
 * @param data
 */
void HAL_I2C_Write_LastByte(I2CInstance *i2cX, uint8_t data);

/**
 * @brief Start I2C, choose I2C slave by sending device address automatically, then send addr and cmd.
 * No STOP signal is issued after `regAddr`.
 *
 * @param i2cX
 * @param regAddr addr or cmd.
 * @param len length of `regAddr`.
 * @return uint32_t return 0 on success, others on failure.
 */
