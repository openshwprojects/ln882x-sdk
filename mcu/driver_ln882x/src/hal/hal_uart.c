...
HAL_StatusTypeDef HAL_UART_Init(UART_DevTypeDef *pUartDev)
{
    ...
    /*
     * When switching the baudrate dynamically, the UART module must be reset.
     */
    if(pUartDev->Instance == UART0){
        HAL_SYSCON_SoftwareResetPeripheral(SW_RST_UART0);
    }else if(pUartDev->Instance == UART1){
        HAL_SYSCON_SoftwareResetPeripheral(SW_RST_UART1);
    }
...
HAL_StatusTypeDef HAL_UART_Deinit(UART_DevTypeDef *pUartDev)
{
    ...
    HAL_UART_INT_Switch_All(pUartDev, false);
    // disable UART module.
...
HAL_StatusTypeDef HAL_UART_BaudRateConfig(UART_DevTypeDef *pUartDev, uint32_t baudRate)
{
    ...
    temp_cal = uart_baudrate_cal(uart_src_clk, baudRate);
    
    if (true != LL_UART_DLAB_Set( pUartDev->Instance, 1)){
        return HAL_ERROR;
    }

    LL_UART_DLL_Set( pUartDev->Instance, (uint8_t) ((temp_cal >> 5U) & 0xFF) );
    LL_UART_DLH_Set( pUartDev->Instance, (uint8_t) ((temp_cal >> 13U) & 0xFF) );
    if (temp_cal & 1) {
        LL_UART_DLF_Set( pUartDev->Instance, (uint8_t)(((temp_cal >> 1) & 0x0F) + 1));
    } else {
        LL_UART_DLF_Set( pUartDev->Instance, (uint8_t)((temp_cal >> 1) & 0x0F));
    }
