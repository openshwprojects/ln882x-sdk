 ...
 */
uint32_t HAL_SYSCON_IdleRegGet(void)
{
    return LL_SYSCON_IdleRegGet();
}

void HAL_SYSCON_IdleRegSet(uint32_t val)
{
    LL_SYSCON_IdleRegSet(val);
}
...
void HAL_SYSCON_GPIO_NoPull(GPIO_Num gpio_num){
    LL_SYSCON_GPIO_NoPull(gpio_num);
}


/**
 * @brief gpio enable/disable analog.
 *
 * @param   GPIO_Num  GPIO_DigitalAnalogSel_t;
 */
void HAL_SYSCON_GPIO_Digital_Analog_Select(GPIO_Num gpio_num, GPIO_DigitalAnalogSel_t mod)
{
    uint16_t bitmap = 0;
    bitmap = LL_SYSCON_DigSel_Get();
    
    if (GPIO_ANALOG_MOD == mod) {
        switch (gpio_num)
        {
            case GPIOA_0:
                bitmap &= ~(1 << 0);
                LL_SYSCON_DigSel_Set(bitmap);
                break;
            case GPIOA_1:
                bitmap &= ~(1 << 1);
                LL_SYSCON_DigSel_Set(bitmap);
...
void HAL_SYSCON_XtalVol_Set(uint8_t Vol)
{
    LL_SYSCON_XtalVol_Set(Vol);
}
void HAL_SYSCON_PmuCfg(void)
{  
    LL_SYSCON_PMUReg_Set(0x8B050500);// down the vol of xtal 1217//bod_vth��bit[12:11]=0b00 2.4v // en_bod�� bit[10]=0b1 enable //(BUCK)  r_vtrim: bit[9:8]=0b01 //00: 1.195 / 01: 1.202  / 10: 1.210 / 11: 1.218
    //LL_SYSCON_PMUCfg_Set(0x20001603);// o_digldo_norvdd: bit[10:8]=0b100; o_digldo_retvdd: bit[14:12]=0b001
    LL_SYSCON_PMUCfg_Set(0x210746fd);// o_digldo_norvdd: bit[10:8]=0b100; o_digldo_retvdd: bit[14:12]=0b110
    LL_SYSCON_Misc_Set(0x00002C0D);

    LL_SYSCON_PMUAvdd_Set(0x00056DB6);// Avdd & PLL & RF ,increase the PLL voltage
}
