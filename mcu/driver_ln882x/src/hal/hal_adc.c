...
void HAL_ADC_Init(ADC_Instance* ADCx, ADC_InitTypeDef* ADC_InitStruct)
{
    ...
    LL_ADC_DMAEN_Set(ADCx, ADC_InitStruct->ADC_DMAEnable);
...
{
    assert_param(IS_ADC_ALL_PERIPH(ADCx));
    assert_param(IS_ADC_CHANMAP(chanx));

    return LL_ADC_DataAddr_Get(ADCx, chanx);
}
...
uint16_t HAL_ADC_GetConversionValue(ADC_Instance* ADCx, uint8_t ChanX)
{
    ...
    switch (ChanX)
    {
    case INTL_ADC_CHAN_0:
        data = LL_ADC_Data0_Get_compensate(ADCx);
        break;
    case INTL_ADC_CHAN_1:
        data = LL_ADC_Data1_Get(ADCx);
        break;
    case EXTL_ADC_CHAN_0:
        data = LL_ADC_Data2_Get(ADCx);
        break;
    case EXTL_ADC_CHAN_1:
        data = LL_ADC_Data3_Get(ADCx);
        break;
    case EXTL_ADC_CHAN_2:
        data = LL_ADC_Data4_Get(ADCx);
        break;
    case EXTL_ADC_CHAN_3:
        data = LL_ADC_Data5_Get(ADCx);
        break;
    case EXTL_ADC_CHAN_4:
        data = LL_ADC_Data6_Get(ADCx);
        break;
    case EXTL_ADC_CHAN_5:
 ...
 * ...
 * 255: input ADC clock divided by 256 * 2
 */
