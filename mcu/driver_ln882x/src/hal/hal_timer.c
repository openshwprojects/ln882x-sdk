...
ln_timer_cb_t g_timer_cb[4]={NULL};
...
void HAL_TIMER_Init(TIMER_InitTypeDef *config)
{
    ...
    assert_param(config->timer_cb.cb_func);
    ...
    if(config->timer_cb.cb_func)
    {
        memcpy(&g_timer_cb[config->index-1], &(config->timer_cb), sizeof(ln_timer_cb_t));
    }
}
...
{
    TIMER_Index index;
    ln_timer_cb_t *timer_cb = NULL;

    for(index=TIMER_1;index<=TIMER_4;index++)
    {
        timer_cb = &(g_timer_cb[index-1]);
        if(HAL_TIMER_Int_Status(index))
        {
            // clear interrupt.
            HAL_TIMER_Int_Clr(index);
            if(timer_cb->cb_func){
                timer_cb->cb_func(timer_cb->arg);
            }
        }
     }
}
