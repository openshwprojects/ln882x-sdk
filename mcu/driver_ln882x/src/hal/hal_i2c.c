...
void HAL_I2C_Read_FIFO(I2CInstance *i2cX, uint8_t *buf, uint32_t length)
{
    ...
    for (i = 0; i < length; i++) {
        while( (times-- >= 1) && (0 == LL_I2C_Rx_FIFO_Level(i2cX)) );
        data = LL_I2C_Data_Get(i2cX);
        buf[i] = data;
    }
}
...
{
    LL_I2C_Data_Cmd_Pack(i2cX, restart, stop, read_write_mode, data);
}
...
void HAL_I2C_ReadData_Polling(I2CInstance *i2cX, uint8_t *buf, uint32_t length)
{
    ...
    for (i = 0; i < length; i++) {
        ...
        fifotimes = 10000;
        while( (fifotimes-- > 1) && (0 == LL_I2C_Rx_FIFO_Level(i2cX)));
...
{
    LL_I2C_Data_Cmd_Pack(i2cX, 0, 1, 0, data);
}

/**
 * @brief Send device command.
 *
 * @param i2cX
 * @param regAddr
 * @param len
