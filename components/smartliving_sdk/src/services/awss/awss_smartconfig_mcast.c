...
void gen16ByteToken()
{
    // int bssid_len = mcast_smartconfig_data.bssid_type_len & 0b11111; // GNU GCC OK, but ARMCC failed.
    int bssid_len = mcast_smartconfig_data.bssid_type_len & 0x1F; // GNU GCC OK, ARMCC OK.
...
int parse_result()
{
    ...
    // offset += mcast_smartconfig_data.bssid_type_len & 0b11111; // GNU GCC OK, but ARMCC failed.
    offset += mcast_smartconfig_data.bssid_type_len & 0x1F; // GNU GCC OK, ARMCC OK.
        ...
        signed char rssi)
{
    ...
    if (ieee80211_has_tods(fc) == ieee80211_has_fromds(fc)) {
        awss_debug("no tods/fromds.\r\n");
        return ALINK_INVALID;
    }

    /* drop frag, more, order*/
    if (ieee80211_has_frags(fc)) {
        awss_debug("drop frag.\r\n");
        return ALINK_INVALID;
    }
    ...
    if (0x1 != dst_mac[0] || 0x0 != dst_mac[1] || 0x5e != dst_mac[2]) {
        ...
        awss_debug("error type, 0x%02X, 0x%02X, 0x%02X\n", dst_mac[0], dst_mac[1], dst_mac[2]);
#endif
    ...
    awss_debug("result --> ALINK_BROADCAST.\r\n");
...
int awss_recv_callback_mcast_smartconfig(struct parser_res *res)
{
    ...
    awss_err("error len = %d, frame_offset = %d\r\n", len, frame_offset);

    len -= frame_offset;
    if ((len != 1) && (len != 18)) {
        awss_err("error len (%d)\r\n", len);
#ifdef VERBOSE_MCAST_DEBUG
    ...
    */
    ...
    if (!memcmp(mcast_src_mac, zero_mac, ETH_ALEN)) {
        ...
        if (memcmp(mcast_src_mac, res->src, ETH_ALEN)) {
            awss_err("filter out case: mac src != dst \r\n");
            return FAILURE_RETURN;
        }
    }
    ...
    awss_debug("mcast: index is %d, 0x%2X, 0x%2X, %d\n", index, payload1, payload2, len);
    ...
    if (0 != receive_record[0]) {
        ...
        awss_debug("remain (%d) == 0 recv ok, > 0 recv not ok!\r\n", remain);
        if (0 == remain) {
            get_all = 1;
            parse_result();
        }
