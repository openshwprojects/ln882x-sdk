...
sc_state_t ln_sc_recv_frame_proc(sc_ctl_t * ctl, uint8_t *data)
{
	...
	if (packet_num <= PACKET_NUM_MAX) {
        uint16_t half_word = iframe->i_addr3[4] | (iframe->i_addr3[5] << 8);
...
static void ln_sc_sniffer_callback(uint8_t *frame, uint32_t len)
{
...
int  ln_smartconfig_start(uint16_t channel_map)
{
	...
	GLOBAL_INT_STOP(); // __disable_irq();
    sc_ctl->is_ch_hopping = LN_TRUE;
	GLOBAL_INT_START(); // __enable_irq();
