...
static bind_status_t sg_app_bind_status = {
    .bind_token_reply_cnt = 0,
    .bind_device_cnt = 0,
    .bind_is_ok = 0
};
...
void downserviceMsgHandler(void *pClient, MQTTMessage *message, void *pUserData)
{
    if (NULL != message) {
        Log_i("topic: %s\r\npayload: %s\r\n", message->ptopic, message->payload);
        cJSON* rootObj = cJSON_Parse(message->payload);
        cJSON* methodObj = NULL;
        cJSON* codeObj = NULL;
        if (NULL != rootObj) {
            methodObj = cJSON_GetObjectItem(rootObj, "method");

            if ((NULL != methodObj) && (cJSON_String == methodObj->type)) {
                if ( memcmp(methodObj->valuestring, XCLOUD_PLATFORM_BIND_TOKEN_REPLY_METHOD, strlen(XCLOUD_PLATFORM_BIND_TOKEN_REPLY_METHOD)) == 0 ) {
                    // method == "app_bind_token_reply"
                    codeObj = cJSON_GetObjectItem(rootObj, "code");
                    if ( (NULL != codeObj) && ( cJSON_Number == codeObj->type) && ( XCLOUD_PLATFORM_BIND_TOKEN_REPLY_CODE == codeObj->valueint ) ) {
                        sg_app_bind_status.bind_token_reply_cnt += 1;
                    }
                } else if (memcmp(methodObj->valuestring, XCLOUD_PLATFORM_METHOD_BIND_DEVICE, strlen(XCLOUD_PLATFORM_METHOD_BIND_DEVICE)) == 0) {
                    // method == "bind_device"
                    sg_app_bind_status.bind_device_cnt += 1;
                }

                Log_i("bind_token_reply_cnt = %d, bind_device_cnt = %d\r\n",
                        sg_app_bind_status.bind_token_reply_cnt,
                        sg_app_bind_status.bind_device_cnt);
 ...
 */
static int _user_sub_downservice(void *client)
{
    if (NULL == client) {
        return -1;
    }
    ...
    const char *fmt = "$thing/down/service/%s/%s";
    #define TOPIC_BUF_LEN       (20 + 64 + 32 + 20)
    char topic_buf[TOPIC_BUF_LEN] = { 0 };
    HAL_Snprintf(topic_buf, TOPIC_BUF_LEN, fmt,
                pTemplate->device_info.product_id,
                pTemplate->device_info.device_name);
    SubscribeParams subParam = {
        .qos = QOS1,
        .on_message_handler = downserviceMsgHandler,
        .on_sub_event_handler = downserviceEventHandler,
        .user_data = NULL
     };
...
/**
 * @brief 发布消息 $thing/up/service/ProductID/DeviceName 上报 token
 *
 * @param pClient
 * @return int
 */
static int _usb_pub_upservice(void *pClient, const char *token)
{
    int retry_cnt = 0;
    ...
    Qcloud_IoT_Client *pMqttClient = (Qcloud_IoT_Client *)pTemplate->mqtt;

    const char *fmt = "$thing/up/service/%s/%s";
    #define TOPIC_BUF_LENGTH    (19 + 64 + 32 + 20)
    char topic_buf[TOPIC_BUF_LENGTH] = { 0 };
    HAL_Snprintf(topic_buf, TOPIC_BUF_LENGTH, fmt,
                    pTemplate->device_info.product_id,
                    pTemplate->device_info.device_name);

    cJSON* rootObj = cJSON_CreateObject();
    cJSON_AddStringToObject(rootObj, "method", "app_bind_token");
    cJSON_AddStringToObject(rootObj, "clientToken", pMqttClient->options.conn_id);

    cJSON* paramObj = cJSON_CreateObject();
    cJSON_AddStringToObject(paramObj, "token", token);

    cJSON_AddItemToObject(rootObj, "params", paramObj);

    char *pJsonDoc = cJSON_Print(rootObj);
    cJSON_Delete(rootObj);
    ...
    for (retry_cnt = XCLOUD_PLATFORM_TOKEN_REPORT_MAX; retry_cnt > 0; retry_cnt--) {
        if (sg_app_bind_status.bind_is_ok) {
            break;
        }

        if (0 > IOT_MQTT_Publish(pTemplate->mqtt, topic_buf, &pubParams)) {
            // 发布消息出错
            Log_e("pub app_bind_token failed!!!\r\n");
        }
        IOT_MQTT_Yield(pTemplate->mqtt, 100);
    }

    HAL_Free(pJsonDoc);

    if (sg_app_bind_status.bind_is_ok) {
        Log_i("---> app_bind_token OK!!!\r\n");
    } else {
        Log_e("---> app_bind_token failed!!!\r\n");
    }

    return 0;
}
/*add user init code, like sensor init*/
static void _usr_init(void *client)
{
    if (NULL == client) {
        return;
    }
    ...
    // step1: 订阅消息 $thing/down/service/ProductID/DeviceName
    _user_sub_downservice(pTemplate);

    // step2: 发布消息，上报token
    // topic: $thing/up/service/ProductID/DeviceName
    // payload: {"method": "app_bind_token", "clientToken": "client-1234",
    //            "params": {"token": "6aa1234a9d529****777aa6e528a0fd"}
    //           }
...
int light_data_template_demo(void)
{
