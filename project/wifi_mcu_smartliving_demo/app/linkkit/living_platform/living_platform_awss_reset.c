...
void living_platform_start_smartconfig_awss(void)
{
    ...
    hal_os_thread_param.name = "devAP_close";
    HAL_ThreadCreate(&close_dev_ap_thread, awss_close_dev_ap, NULL, &hal_os_thread_param, NULL);

    memset(&hal_os_thread_param, 0, sizeof(hal_os_thread_param_t));
    hal_os_thread_param.stack_size = 1024 * 8;
    hal_os_thread_param.name = "smartcfg";
