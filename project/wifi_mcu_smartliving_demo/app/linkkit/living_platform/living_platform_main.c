...
static iotx_linkkit_dev_meta_info_t *living_platform_main_init(living_platform_ctx_t *living_platform_ctx)
{
        ...
        (0 == register_type && (0 == strlen(p_master_meta->device_secret))) || (0 == strlen(p_master_meta->product_secret)))
    {
        while (1)
        {
            LOG(LOG_LVL_ERROR, "Master meta info is invalid...\r\n");
            LOG(LOG_LVL_ERROR, "pk[%s]\r\n", p_master_meta->product_key);
            LOG(LOG_LVL_ERROR, "ps[%s]\r\n", p_master_meta->product_secret);
            LOG(LOG_LVL_ERROR, "dn[%s]\r\n", p_master_meta->device_name);
            LOG(LOG_LVL_ERROR, "ds[%s]\r\n", p_master_meta->device_secret);
            HAL_SleepMs(2000);
        }
    }
...
extern void living_platform_start_smartconfig_awss(void);
...
int living_platform_main(void *paras)
{
    ...
    {
        LOG(LOG_LVL_ERROR, "OOPS:living_platform_main_init failed");
        return -1;
    }
    ...
    living_platform_start_smartconfig_awss();
    ...
    do
    {
        ...
        {
            LOG(LOG_LVL_ERROR, "IOT_Linkkit_Open Failed, retry after 5s...\r\n");
            HAL_SleepMs(5000);
        }
    ...
    do
    {
        ...
        if (res < 0)
        {
            LOG(LOG_LVL_ERROR, "IOT_Linkkit_Connect Failed, retry after 5s...\r\n");
            HAL_SleepMs(5000);
        }
    } while (res < 0);
